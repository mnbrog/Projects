...
public:
  ...
  void insertNode(keytype k, valuetype v) {
    Node<keytype,valuetype> *z = new Node(k, v, RED, nullptr, nullptr, nullptr);
    ...
    if (y == nullptr) {
      ...
      y->left   = z ;
      z->parent = y ; //all non-root nodes must have parent to be "in" the tree
    } else {
      y->right  = z;
      z->parent = y ; //all non-root nodes must have parent to be "in" the tree
    }
    
    if (k == "H") cout << "insert H, parent is : " << z->parent->key << endl ;
    insertFixup(z);
  }
...
//private:
public :
  Node<keytype,valuetype> *buildTree(keytype k[], valuetype v[], int start, int end, Node<keytype,valuetype> *parent) {
    if (start > end) {
      return nullptr;
    }
    
    int mid = (start + end) / 2;
    Node<keytype,valuetype> *node = new Node<keytype,valuetype>(k[mid], v[mid], BLACK, nullptr, nullptr, parent);
  ...
  void printInorder(Node<keytype,valuetype> *node) {
    if (node != nullptr) {
...
void insert(keytype k, valuetype v) {
    Node<keytype, valuetype>* z = new Node<keytype, valuetype>(k, v,RED,nullptr,nullptr,nullptr) ;
    insertNode(k,v);
    count ++ ;
}
...
keytype select(int pos) {
    ...
    Node<keytype, valuetype>* node = selectNode(root, pos);
    return node->key;
}

Node<keytype, valuetype>* selectNode(Node<keytype, valuetype>* node, int pos) {
...
//function prototype must at least be insidet he class definition
...
void preorder(Node<keytype,valuetype> *node) {
    if (node == NULL) {
        return;
    }
